...
def get_rates():
    ...
    try:
        ...
        if len(rates) <= 1:
            ...
            if len(rates) <= 1:
                # If still no rates, download the HTML for local parsing
                try:
                    html_content = driver.page_source
                    # Save the HTML content for debugging if needed
                    # with open("vickygold.html", "w", encoding="utf-8") as f:
                    #     f.write(html_content)

                    # Extract rates using simple string operations
                    from bs4 import BeautifulSoup

                    soup = BeautifulSoup(html_content, "html.parser")
                    # Find all rate tables
                    rate_tables = soup.select(
                        "#divProduct #tblLiveRates, #silverproduct #tblLiveRates"
                    )

                    for table in rate_tables:
                        try:
                            symbol_elem = table.select_one(".tabletitle #GoldSymbol")
                            rate_elem = table.select_one(".rate #GoldSell")
                            ...
                            print(f"Error in BeautifulSoup extraction: {e}")
                            continue
                except ImportError:
                    print(
